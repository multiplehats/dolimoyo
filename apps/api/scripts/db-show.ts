...
console.log(`  scrapers:       ${s.activeScrapers} active (${s.cssScrapers} css)`)
console.log(`  discovery runs: ${s.discoveryRuns}`)
console.log(`  refresh runs:   ${s.extractRuns}`)
...
if (s.byLocation.length > 0) {
  ...
  for (const loc of s.byLocation.sort((a, b) => b.sources - a.sources)) {
    ...
    console.log(
      `    ${loc.label.padEnd(20)} ${String(loc.sources).padStart(3)} src (${String(loc.cssScrapers).padStart(2)} css) · ${String(loc.events).padStart(4)} events · ${String(loc.perennials).padStart(3)} perennial (${noisePct}%)`,
    )
  }
  console.log()
}
